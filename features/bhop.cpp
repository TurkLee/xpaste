...
void BunnyHop::HumanBhop(CUserCmd* cmd) {
	...
	else if ((rand() % 100 > g_Options.bhop_hit_chance
		&& hops_restricted < g_Options.hops_restricted_limit)
		|| (g_Options.max_hops_hit > 0
			&& hops_hit > g_Options.max_hops_hit))
	{
		cmd->buttons &= ~IN_JUMP;
		hops_restricted++;
		hops_hit = 0;
