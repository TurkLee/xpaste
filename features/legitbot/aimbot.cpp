...
int get_group()
{
	...
	if (!pWeapon || !(pWeapon->IsSniper() || pWeapon->IsPistol() || pWeapon->IsRifle() || pWeapon->IsZeus() || pWeapon->IsKnife()))
	{
		return 0;
	}
	if (pWeapon->IsZeus())
		...
		return 1;
	else if (pWeapon->GetCSWeaponData()->iWeaponType == WEAPONTYPE_SUBMACHINEGUN)
		return 2;
	else if (pWeapon->GetCSWeaponData()->iWeaponType == WEAPONTYPE_SHOTGUN)
		return 3;
	else if (pWeapon->GetCSWeaponData()->iWeaponType == WEAPONTYPE_MACHINEGUN)
		return 4;
	else if (pWeapon->GetCSWeaponData()->iWeaponType == WEAPONTYPE_RIFLE)
		return 5;
	else if (pWeapon->GetCSWeaponData()->iWeaponType == WEAPONTYPE_SNIPER_RIFLE)
		return 6;
	else if (pWeapon->GetCSWeaponData()->iWeaponType == WEAPONTYPE_KNIFE)
		return 7;
}
//--------------------------------------------------------------------------------
bool Aimbot::IsEnabled(CUserCmd* pCmd) {
	...
	if (g_Options.settings_category == 0)
		...
		settings = g_Options.legitbot_items_groups[get_group()];
	...
	for (int i = 1; i < g_EngineClient->GetMaxClients(); ++i) {
		...
		for (int bone = fromBone; bone <= toBone; bone++) {
			...
			if (!g_LocalPlayer->CanSeePlayer(player, eVecTarget)) {
				...
				damage = CAutoWall::Get().CanHit(eVecTarget);
