...
static auto erase_override_if_exists_by_index(const int definition_index) -> void
{
	...
	{
		auto& icon_override_map = g_Options.m_icon_overrides;
...
{
	return g_Options.m_icon_overrides.count(original) ? g_Options.m_icon_overrides.at(original).data() : nullptr;
}
...
void skins::on_frame_stage_notify(bool frame_end)
{
	...
	if (frame_end)
	{
		...
		const auto glove_config = &g_Options.m_items[GLOVE_T_SIDE];
		...
		if (glove_config && glove_config->definition_override_index)
		{
			if (!glove)
			{
				...
				assert(glove);
				{
					static auto set_abs_origin_addr = Utils::PatternScan(GetModuleHandle("client.dll"), "55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8");
	...
	else
	{
		...
		for (int i = 0; weapons[i].IsValid(); i++)
		{
			...
			const auto active_conf = &g_Options.m_items[is_knife(definition_index) ? WEAPON_KNIFE : definition_index];
