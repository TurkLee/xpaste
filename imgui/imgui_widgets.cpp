...
bool ImGui::OwOButtonXD(const char* label, const ImVec2& size_arg, bool inuse, ImGuiButtonFlags flags)
{
	...
	if (g_Options.selected_theme == 0)
	{
		auto turuncu = ImColor(255, 255, 255);
		auto koyuturuncu = ImColor(255, 0, 0);
		if (inuse)
		{

			window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Max.y - 2), ImVec2(bb.Max.x, bb.Max.y - 0), turuncu, turuncu, turuncu, turuncu);

		}
		else
		{
			window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Max.y - 2), ImVec2(bb.Max.x, bb.Max.y - 0), koyuturuncu, koyuturuncu, koyuturuncu, koyuturuncu);
		}
	}
	if (g_Options.selected_theme == 1)
	{
		auto turuncu2 = ImColor(255, 255, 255);
		auto koyuturuncu2 = ImColor(255, 0, 0);
		if (inuse)
		{

			window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Max.y - 2), ImVec2(bb.Max.x, bb.Max.y - 0), turuncu2, turuncu2, turuncu2, turuncu2);

		}
		else
		{
			window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Max.y - 2), ImVec2(bb.Max.x, bb.Max.y - 0), koyuturuncu2, koyuturuncu2, koyuturuncu2, koyuturuncu2);
		}
	}
...
void ImGui::CheckMarkxD(ImVec2 pos, ImU32 col, float sz)
{
    ...
    float thickness = ImMax(sz / 5.0f, 1.0f); // yazı kalınlığı
    sz -= thickness * 0.5f; // kalınlık eklenti 1
    pos += ImVec2(thickness * 0.25f, thickness * 0.25f); // kalınlık eklenti 2

    float third = sz / 3.0f;// 3
    float bx = pos.x + third;// x
    float by = pos.y + sz - third * 0.5f;// y
    window->DrawList->PathLineTo(ImVec2(bx - third, by - third));// drawlist modül
    window->DrawList->PathLineTo(ImVec2(bx, by));// drawlist modül
    window->DrawList->PathLineTo(ImVec2(bx + third * 2, by - third * 2));// drawlist modül
    window->DrawList->PathStroke(col, false, thickness);// drawlist modül
}

void ImGui::FlowzerCheckMark(ImVec2 pos, ImU32 col, float sz)
{
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 5.0f ? style.ItemInnerSpacing.x + label_size.x : 5.0f), label_size.y + style.FramePadding.y * 3.0f)); //arasındaki boşluk
    ...
    ImGui::SameLine();
    ImGui::SameLine();
    RenderText(ImVec2(50 + check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y - 6), label);
    ...
    if (ImGui::IsItemHovered()) {
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.447f, 0.463f, 0.490f, 1.000f), ImVec4(0.447f, 0.537f, 0.855f, 1.000f), t));
        top_bg2 = IM_COL32(255, 255, 255, 50);

    }
    else {


        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.447f, 0.463f, 0.490f, 0.1f), ImVec4(0.447f, 0.537f, 0.855f, 1.f), t));
    }
    ...
    if (ImGui::IsItemHovered()) {
        draw_list->AddCircleFilled(ImVec2(p.x + radius2 - 6.f + t * (width + 13.f - radius2 * 2.0f), p.y + radius2 / 2), radius2 - 0.5f, IM_COL32(255, 255, 255, 50), 30); // saydam top
...
void ImGui::DeadlySeparator(const char* label)
{
    const auto size = ImGui::CalcTextSize(label);
    auto draw_list = ImGui::GetWindowDrawList();
    const auto width = ImGui::GetContentRegionAvailWidth();
    const auto cursor = ImGui::GetCursorScreenPos();
    const auto pos = ImVec2(cursor.x + width / 800 - size.x / 600, cursor.y); //Yazı konumu
    //ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.00f, 1.00f, 1.00f, 0.50f));
    const auto col = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Text)); //Rengi
    //ImGui::PopStyleColor();
    draw_list->AddText(pos, col, label); //Yazı ekleme fonksiyonu
    draw_list->AddLine(ImVec2(pos.x + size.x + 5.f, cursor.y + size.y / 2), ImVec2(cursor.x + width, cursor.y + size.y / 2), col); //Sağ çizgi
    ImGui::NewLine();
}
...
bool ImGui::ImageButtonWithText(ImTextureID texId, const char* label, const char* desc, const ImVec2& imageSize, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col) {
    ImGuiWindow* window = GetCurrentWindow();
    float height = ImGui::GetFrameHeight();
    ...
    RenderText(ImVec2((p.x + width) + 30, (p.y + height) - 19), label);
    ImGui::PopStyleColor();

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.00f, 1.00f, 1.00f, 0.90f));
    RenderText(ImVec2((p.x + width) + 30, (p.y + height) - 5), desc);
    ImGui::PopStyleColor();
    return pressed;
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.сука комп хуйня пиздец долгий этот код лучше прошлого?
    ...
    */
    ...
    //RenderText(frame_bb.Min + ImVec2(frame_bb.Max.x - frame_bb.Min.x - 8, (grab_bb.Max - grab_bb.Min).y / 2.f + 12), value_buf[2], value_buf_end[2]); закрой
    ...
    // так точка неправильно отрисовывается, а не та херня
    //ImVec2(grab_bb.Max.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.f))
    //window->DrawList->AddCircleFilled(grab_bb.Min + ((grab_bb.Max - grab_bb.Min) / 2.f), 8.f, ImColor(255, 255, 255), 64); // лол блять
    ...
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f)) - ImVec2(0, 0.7), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.f)) + ImVec2(0, 0.7), ImColor(64, 64, 64), true, style.FrameRounding);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f)) - ImVec2(0, 0.7), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.f)) + ImVec2(0, 0.7), ImColor(114, 137, 218), true, style.FrameRounding);
    if (g_Options.theme == false) {
        if (ImGui::IsItemHovered()) {
            window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f)), 17.f, ImColor(114, 137, 218, 50), 30);
    ...
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f)), 8.f, ImColor(255, 255, 255), 30);

    //ImVec4(0.447f, 0.463f, 0.490f, 1.000f), ImVec4(0.447f, 0.537f, 0.855f, 1.000f)
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value. (grab_bb.Max.x, эта строчка значит что он рисует белую хуйню до значения grab_bb.max
    if (label_size.x > 0.0f)
