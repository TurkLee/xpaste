...
namespace notify
{
	void notify_events(IGameEvent* event) {
		...
		/*	if (strstr(event->GetName(), "player_hurt"))
			{
				...
				if (attacker_id == g_LocalPlayer->EntIndex() && userid_id != g_LocalPlayer->EntIndex()) {
					...
					if (g_Options.eventlogger_hurt) {
						screen::notify("damage", ss.str(), g_Options.accent_color);
						console::notify("damage", ss.str(), g_Options.accent_color);
					}
				}
		...
		if (strstr(event->GetName(), "item_purchase"))
		{
			...
			if (g_Options.eventlogger_player_buy) {
				screen::notify("buy", ss.str(), g_Options.accent_color);
				console::notify("buy", ss.str(), g_Options.accent_color);
			}
		}
		...
		if (strstr(event->GetName(), "bomb_beginplant"))
		{
			...
			if (g_Options.eventlogger_planting) {
				screen::notify("info", ss.str(), g_Options.accent_color);
				console::notify("info", ss.str(), g_Options.accent_color);
			}
		}
