...
CCSGOPlayerAnimState* C_BasePlayer::GetBasePlayerAnimstate()
{
...
{
	return !InDangerzone() && this != g_LocalPlayer && this->m_iTeamNum() == g_LocalPlayer->m_iTeamNum();
}
...
{
	return CallVFunction<const char* (__thiscall*)(C_BaseEntity*)>(this, 383)(this);
}
...
{
	static auto fn = reinterpret_cast<bool(__thiscall*)(void*, int)>(Utils::PatternScan(GetModuleHandle("client.dll"), "55 8B EC 83 EC 10 53 56 57 8B F9 8B 87"));
...
{
	static auto fn = reinterpret_cast<void(__thiscall*)(int)>(Utils::PatternScan(GetModuleHandle("client.dll"), "55 8B EC 56 57 8B F9 8B B7 ? ? ? ? 8B C6"));
...
int C_BaseCombatWeapon::get_item_definition_index()
{
	if (!this)
		return 0;
...
bool C_BasePlayer::is_flashed(int min_alpha)
{
...
bool C_BasePlayer::CanSeePlayer(C_BasePlayer* player, int hitbox, bool smokecheck)
{
	...
	if (IsLineGoesThroughSmoke(g_LocalPlayer->GetEyePos(), endpos) && smokecheck)
		return false;
...
{
	if (IsLineGoesThroughSmoke(GetEyePos(), pos) && smokecheck)
		return false;
	CGameTrace tr;
	Ray_t ray;
	CTraceFilter filter;
...
float C_BasePlayer::can_see_player(const Vector& pos)
{
	CGameTrace tr;
	Ray_t ray;
	CTraceFilter filter;
...
bool C_BaseCombatWeapon::IsAllWeapons() {
	switch (GetCSWeaponData()->iWeaponType) {
	case WEAPONTYPE_RIFLE:
		return true;
	case WEAPONTYPE_SUBMACHINEGUN:
